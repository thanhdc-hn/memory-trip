...
import { teamService, type CreateTeamInput, type UpdateTeamInput, type Team } from '@/services/team.service'
...
export function useTeams() {
  ...
  const toggleLockMutation = useMutation({
    ...
    onMutate: async ({ id, isLocked }) => {
      ...
      if (previousTeams) {
        queryClient.setQueryData<Team[]>(['teams'], 
          previousTeams.map(t => t.id === id ? { ...t, is_locked: isLocked } : t)
        )
      }
  ...
  return {
    ...
    toggleTeamLock: (id: string, isLocked: boolean) => toggleLockMutation.mutateAsync({ id, isLocked }),
    refetch
  }
}
